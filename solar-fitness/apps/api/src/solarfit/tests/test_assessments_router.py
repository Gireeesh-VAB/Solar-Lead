...
def db_backed_dependencies(sqlite_engine, monkeypatch):
    ...
    monkeypatch.setattr(calibration_module, "get_session", lambda: session_local())
    monkeypatch.setattr(ml_score_module, "get_session", lambda: session_local())
...
def stub_pipeline(monkeypatch, make_site):
    ...
    monkeypatch.setattr(
        router_module.sites_repo, "get", lambda site_id: site if site_id == site.id else None
    )

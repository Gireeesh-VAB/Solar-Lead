...
Person 1 reuses the same factory for repositories/sites.py; Person 4's
repositories/usn_uploads.py, calibration.py, and ml_models.py all use
the same get_session() via `with get_session() as session: ...`, same
as everyone else).
